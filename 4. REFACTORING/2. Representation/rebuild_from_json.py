...
Workflow:
1. Legge i file JSON aggiornati dalla directory "unified_output"
...
INPUT_JSON_DIR = "output_updated_json"  # Directory con i JSON aggiornati
REBUILD_DIR = "rebuild_from_updated_json"  # Nuovo folder per i risultati
TARGET_CANVAS_SIZE = (3000, 3000)  # Dimensioni canvas
...
class FloorplanReconstructor:
    ...
    def process_json_file(self, json_path: str, output_dir: str) -> bool:
        ...
        base_name = base_name.replace('_graph', '')  # Rimuovi "_graph" dal nome
    ...
    def process_all_json_files(self, input_dir: str, output_dir: str) -> None:
        """Processa tutti i file JSON nella directory specificata."""
        print("🚀 Avvio ricostruzione immagini dai file JSON aggiornati")
        
        # Crea directory di output (può essere un path relativo o assoluto)
        ...
        print(f"📁 Directory output: {full_output_dir}")
        
        # Trova tutti i file JSON aggiornati
        json_pattern = os.path.join(input_dir, "*_graph_updated.json")
        json_files = sorted(glob.glob(json_pattern))
        
        if not json_files:
            print(f"❌ Nessun file JSON aggiornato trovato in {input_dir}")
            print(f"   Pattern cercato: {json_pattern}")
            return
        ...
        for json_path in json_files:
            try:
                if self.process_json_file(json_path, full_output_dir):
                    processed_count += 1
                else:
                    failed_count += 1
            except Exception as e:
...
Esempi di utilizzo:
  ...
  python rebuild_from_json.py --canvas-size 1024 1024  # Specifica dimensioni canvas
    ...
    parser.add_argument(
        ...
        help='Dimensioni del canvas (larghezza altezza) - default: 3000 3000'
    )
    
    args = parser.parse_args()
    
    print("Ricostruzione immagini piantine dai file JSON aggiornati")
    print("=" * 60)
    
    # Determina la directory di input (relativa allo script)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    input_json_dir = os.path.join(script_dir, INPUT_JSON_DIR)
    
    # Verifica che la directory esista
    if not os.path.exists(input_json_dir):
        print(f"❌ Directory {input_json_dir} non trovata!")
        print(f"   Assicurati di aver eseguito prima generate_updated_json.py")
        return 1
    
    # Usa dimensioni canvas target se non specificate diversamente
    canvas_size = tuple(args.canvas_size) if args.canvas_size != [3000, 3000] else TARGET_CANVAS_SIZE
    
    # Crea e esegui il ricostruttore
    reconstructor = FloorplanReconstructor(canvas_size=canvas_size)
    reconstructor.process_all_json_files(input_json_dir, REBUILD_DIR)
