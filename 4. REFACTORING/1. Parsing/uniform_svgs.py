...
def uniform_svg_to_template(source_svg: str, template_svg: str, output_svg: str):
    """Uniforma un SVG alla struttura standard."""
...
def main():
    ...
    # Crea un SVG template virtuale con le dimensioni target
    # Non serve più un file template reale
    for svg_file in svg_files:
        source_path = os.path.join(INPUT_DIR, svg_file)
        output_path = os.path.join(OUTPUT_DIR, svg_file)
        
        # Uniforma lo SVG (template_svg non viene più utilizzato, solo le dimensioni target)
        uniform_svg_to_template(source_path, None, output_path)
