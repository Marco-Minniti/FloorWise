...
def split_segments_at_intersection_points(segments, all_other_points, tolerance=0.1):
    """
    Divide i segmenti quando incontrano punti di altri segmenti (anche di colore diverso).
    
    Args:
        segments: Lista di segmenti da controllare
        all_other_points: Insieme di tutti i punti di altri segmenti
        tolerance: Tolleranza per determinare se un punto giace su un segmento
    
    Returns:
        Lista di segmenti divisi
    """
    if not segments or not all_other_points:
        return segments
    ...
    for seg in segments:
        ...
        # Trova tutti i punti che giacciono su questo segmento
        intersecting_points = []
        for point in all_other_points:
            # Verifica che il punto non sia uno degli estremi del segmento
            if point != p1 and point != p2:
                if point_on_segment(point, seg, tolerance):
                    # Calcola la distanza dal punto p1 per ordinare i punti
                    dist = np.sqrt((point[0] - p1[0])**2 + (point[1] - p1[1])**2)
                    intersecting_points.append((point, dist))
        
        if not intersecting_points:
            # Nessun punto di intersezione, mantieni il segmento originale
            split_segments.append(seg)
        else:
            # Ordina i punti per distanza da p1
            intersecting_points.sort(key=lambda x: x[1])
            
            # Crea segmenti consecutivi
            current_point = p1
            for (intersection_point, _) in intersecting_points:
                # Aggiungi il segmento da current_point a intersection_point
                split_segments.append((current_point, intersection_point))
                current_point = intersection_point
            
            # Aggiungi l'ultimo segmento da current_point a p2
            split_segments.append((current_point, p2))
...
def merge_collinear_segments_conservative(graph, points_set, original_segments, all_other_points=None):
    """
    ...
    Infine, divide i segmenti quando incontrano punti di altri segmenti.
    
    Args:
        graph: Grafo di adiacenza dei segmenti
        points_set: Insieme di punti per questo colore
        original_segments: Segmenti originali per questo colore
        all_other_points: Insieme di tutti i punti di altri colori (per la divisione)
    """
    ...
    # Dividi i segmenti quando incontrano punti di altri segmenti
    if all_other_points is not None:
        new_segments = split_segments_at_intersection_points(new_segments, all_other_points)
...
def process_svg(input_file, output_file):
    ...
    # Raccogli tutti i punti di tutti i colori per la divisione dei segmenti
    all_points_all_colors = set()
    for points in points_by_color.values():
        all_points_all_colors.update(points)
    ...
    for color in all_colors:
        ...
        # Prepara i punti di altri colori per la divisione
        other_color_points = all_points_all_colors - points
        
        # Unisci i segmenti collineari (approccio conservativo)
        kept_points, new_segments = merge_collinear_segments_conservative(graph, points, segments, other_color_points)
...
def main():
    ...
    output_dir = os.path.join(script_dir, 'out_remove_collinear_points')
    ...
    for svg_file in svg_files:
        ...
        # Usa lo stesso nome del file per l'output (sovrascrive)
        output_file = os.path.join(output_dir, svg_file)
