...
class OnionAlgorithm:
    ...
    def _coarse_to_fine_search(
        ...
        # Binary search in fine range with step of 5 (smaller step for better precision)
        fine_deltas = list(range(int(fine_lo), int(fine_hi) + 1, 5))
        ...
        for delta_fine in fine_deltas:
            ...
            try:
                ...
                if satisfies:
                    ...
                    # Check for overshoot
                    if operator == ">=" and new_area > (target_area + self.tolerance):
                        break  # Overshoot
    ...
    def solve_multiple_solutions(
        ...
        for solution_idx in range(num_solutions):
            ...
            # Use a random seed to get different solutions
            random.seed(solution_idx)
            ...
            if not self._is_constraint_satisfied(current_house, constraint):
                ...
                if all_candidate_walls:
                    ...
                    for wall_id, wall, adjacent_room in all_candidate_walls:
                        ...
                        partition_ok = is_partition_ok and not is_self_connecting and not is_hard and not is_avoided
                ...
                # Filter to get only OK walls (partition, not HARD, not self-connecting, not avoided)
                valid_walls = []
                for wall_id, wall, adjacent_room in all_candidate_walls:
                    is_load_bearing = wall.load_bearing if wall else False
                    if is_load_bearing:
                ...
                if not valid_walls:
                    ...
                    print(f"     No valid walls (all excluded: load-bearing, HARD, self-connecting, or avoided)")
                    continue
                ...
                for attempt in range(max_attempts):
                    ...
                    # Use hierarchical search (coarse→fine) instead of trying all deltas
                    optimal_delta = self._coarse_to_fine_search(
                        current_house, wall_id, best_direction, constraint_room,
                        target_area, current_area, operator
                    )
                    
                    if optimal_delta is not None:
                        op = {"op": "move_wall", "wall": wall_id, "delta": optimal_delta}
                        new_house = apply(current_house, op)
                        new_area = room_area(new_house, constraint_room)
                        sign_str = "+" if optimal_delta > 0 else "-"
                        print(f"         Found optimal delta {sign_str}{abs(optimal_delta)}: {current_area:.2f} → {new_area:.2f} m²")
                        if self._check_constraint_with_tolerance(new_area, constraint):
                            print(f"         Moved {wall_id} by {optimal_delta} - constraint satisfied")
    ...
    def _solve_preserve_constraints(
        ...
        # Get Layer 1 rooms (adjacent to target)
        layer1_rooms = self._get_layer_rooms(current_house, target_room, 1)
        print(f"   Layer 1 rooms: {layer1_rooms}")
        
        # Process each preserve constraint in the specific order from the document
        # 1. BAGNO3 (== bagno_area)
        # 2. STUDIO5 (== studio5_area) 
        # 3. DISIMPEGNO (>= 52)
        
        constraint_order = [
            "s#room_3#BAGNO",    # BAGNO3
            "s#room_5#STUDIO",   # STUDIO5
            "s#room_4#DISIMPEGNO" # DISIMPEGNO
        ]
        
        for room_id in constraint_order:
            if room_id in layer1_rooms and room_id not in self.hard_rooms:
                # Find the constraint for this room
                room_constraint = None
                for constraint in preserve_constraints:
                    if len(constraint) >= 3 and constraint[1][1] == room_id:
                ...
                if room_constraint:
                    ...
                    # Try to satisfy the constraint using the specific wall sequence
    ...
    def _satisfy_room_constraint_from_rooms(
        ...
        if all_candidate_walls:
            ...
            for wall_id, wall, source_room in all_candidate_walls:
                ...
                partition_ok = is_partition_ok and not is_self_connecting and not is_hard
        ...
        # Filter to get only OK walls (partition, not HARD, not self-connecting)
        valid_walls = []
        for wall_id, wall, source_room in all_candidate_walls:
            is_load_bearing = wall.load_bearing if wall else False
            if is_load_bearing:
        ...
        if not valid_walls:
            print(f"     No valid walls (all excluded: load-bearing, HARD, or self-connecting)")
            return False, operations_applied, current_house
        ...
        for attempt in range(max_attempts):
            ...
            # Use hierarchical search to find optimal delta
            optimal_delta = self._coarse_to_fine_search(
                current_house, wall_id, best_direction, room_id,
                target_area, current_area, operator
            )
            ...
            if optimal_delta is not None:
                ...
                if self._check_constraint_with_tolerance(new_area, constraint):
                    print(f"       Found solution: moved {wall_id} by {optimal_delta}")
                    operations_applied.append(op)
...
def solve_with_onion_algorithm(
    ...
    if num_solutions == 1:
        return algorithm.solve(house, target_room, goal_constraints, preserve_constraints)
    else:
        return algorithm.solve_multiple_solutions(house, target_room, goal_constraints, preserve_constraints, num_solutions)
